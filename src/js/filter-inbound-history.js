...
App.filterInboundHistory = (function () {
  ...
  function checkCell(row) {
    ...
    checkbox.setAttribute('aria-label', row.productCode + ' / 製造番号 ' + row.serialNo + 'を選択');
  ...
  function syncSelectionUi() {
    var deletableRows = App.store.listFilterInboundHistory(currentFilter(), sortOrder).filter(isDeletable);
  ...
  function init() {
    ...
    selectAllCheckbox = document.getElementById('filter-inbound-history-select-all');
    bulkDeleteButton = document.getElementById('filter-inbound-history-bulk-delete');
