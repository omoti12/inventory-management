...
App.inboundHistory = (function () {
  ...
  function saveEdit(row, inputs) {
    ...
    App.store.updateItem(row.id, data).then(function (result) {
      if (!result.ok) {
        var message = result.message || (result.errors && (result.errors.quantity || result.errors.receivedBy || result.errors.enteredBy)) || '更新に失敗しました。';
  ...
  function renderRow(row) {
    ...
    if (editingId === row.id) {
      ...
      saveButton.addEventListener('click', function () {
        saveEdit(row, { quantity: qtyInput, arrivalDate: dateInput, receivedBy: byInput, enteredBy: enteredByInput, remarks: remarksInput });
      });
