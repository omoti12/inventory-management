...
今のところ`inbound.js`（`App.views.inbound.onHide = resetForm`）と`filter-inbound.js`
（`App.views['filter-inbound'].onHide = resetForm`）で使っている。入庫は他の画面（在庫一覧・
商品管理など）を頻繁に行き来しながら使うため、入力途中のまま他画面に移動して戻ってくると
古い入力が残っていて紛らわしい、という声を受けて追加した。
...
- **ヘッダーの「更新」ボタン**（`App.ui.refreshCurrentView()`は今表示中の画面の`onShow`を
  呼び直す）。入庫画面を開いたまま更新ボタンを押しても、入力中の内容が消えては困る。
...
| `listShippedByNames()` | 過去の出庫履歴（通常品・フィルター品問わず）から「出庫した人」の名前を重複無く、新しい順で返す（出庫フォーム等のプルダウン候補用） | 同期 |
...
出庫の担当者はいつも同じ数人であることが多いため、「出庫した人」欄も出荷先コード等と
同じように`<datalist>`で入力候補を出すようにした。`store.js`の`listShippedByNames()`が、
これまでの全出庫履歴（通常品・フィルター品を問わず`shipments`全件）から`shippedBy`を
重複無く集めて返す（出庫日時の新しい順）。出荷先マスタのような専用の登録画面は無く、
過去に実際に入力された名前がそのまま候補になる。

- **出庫・フィルター出庫フォーム**（`shipping.js`/`filter-shipping.js`）:
  `refreshShippedByNames()`が候補を作り直す。画面表示時（`onShow`）・初期化時に加え、
  出庫成功直後にも呼び直す（新しい名前で出庫したら、続けて他の商品を出庫する時にもすぐ
  候補に出るように）。
- **出庫履歴・フィルター出庫履歴の編集ダイアログ**（`ui.js`の`editShipment()`）:
  ダイアログを開くたびに同じ候補を作り直す（`shipment-edit-shipped-by-list`）。
- `<input list="...">`と`<datalist>`の組み合わせなので、候補に無い名前もそのまま自由入力できる
  （出荷先コード欄などと同じ扱い）。
