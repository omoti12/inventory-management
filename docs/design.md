...
（出荷日・伝票入力担当者コード・出荷先コード・出荷先小番・出荷先名1・出荷先名2・受注番号1〜3・
商品コード）は先方の取込画面の項目順に合わせている。日付は「YYYY/M/D」形式（0埋めしない）で、
`formatDateTime()`とは別の`formatExternalDate()`を使う（先方システムの実例CSVがこの形式だった
ため）。伝票入力担当者コードは、先方には担当者コードから氏名を引く仕組みがあるがこちらには
コード体系が無いため、「出庫した人」の名前をそのまま入れている。
