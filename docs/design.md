...
使い方の説明だけの静的なページなので、他の画面と違い専用のJSモジュール（`App.views.help`
のような登録）を作っていない。`ui.js`の`VIEWS`配列に`'help'`を加え、`index.html`に
`data-view="help"`のタブと`#view-help`のセクションを用意するだけで、既存の`showView()`
の仕組み（タブのハイライト・表示切り替え・`localStorage`への記憶）にそのまま乗る。
`App.views['help']`が存在しなくても`showView()`は`onShow`の呼び出しを安全にスキップする
（`view && typeof view.onShow === 'function'`のガードがあるため）。

タブは「通常」「フィルター」に続く3つ目のグループ「その他」に置いている
（`.tabs-nav`は`flex-direction: column`で行が縦に並ぶ作りなので、グループを追加しても
崩れない）。説明文だけのカードを並べる構造のため、フォームや検索欄用の`.card__head`/
`.form`とは別に、`.card__body`（内側にパディングを持つ、見出し・段落・リスト用の
汎用クラス）をCSSに追加した。
...
| ヘルプ | システムの使い方の説明（静的なテキストのみ、専用のJSモジュール無し） |
