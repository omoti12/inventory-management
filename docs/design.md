...
- 外部 CDN・Web フォント・画像を使わず、オフラインで開ける（QRコード読み取りライブラリのみ `src/js/vendor/` に取り込み済みで、実行時に外部へアクセスしない）。
- 見た目はモダン（白基調・アクセント1色・余白多め・薄い影）。
- QRコード読み取り（カメラ）を使う場合のみ、`https`、または `http://localhost` などの**セキュアなコンテキスト**が必要。
...
src/js/scanner.js         QRコード読み取り（カメラ。BarcodeDetector API、無ければ vendor/zxing.min.js にフォールバック）
...
- `migrateLegacyItems()`：商品マスタを持たない最古形式の item を商品マスタ参照に変換したうえで、
  製造番号・入荷月ベースの item を 数量・入庫した人・入荷日ベースに変換する
  （`quantity = 1`、`arrivalDate = 旧入荷月 + '-01'`、`serialNo` は破棄）。
  受注番号・案件番号（`orderNo` / `projectNo`）は廃止したフィールドなので、残っていれば無条件に取り除く。
...
| `addItem(data)` | 通常品を入庫登録する（商品コード自由入力・数量・入庫した人を検証） |
| `addFilterItem(data)` | フィルター品を入庫登録する（商品選択・製造番号・入荷日付を検証） |
...
| フィルター入庫 | フィルター商品の選択、製造番号・入荷日付の入力、QRコード読み取り |
...
（フィルター商品管理の登録・更新・削除に追従）。製造番号欄にはQRコード読み取りボタンがあり、
...
| 種別 | 商品コード | 製品名 | 在庫 |
| --- | --- | --- | --- |
| 通常品 | ABC-100 | アングルブラケット ABC-100 | 数量10（別途2個出庫済み） |
| 通常品 | DEF-200 | フランジ DEF-200 | 数量5 |
| 通常品 | GH-3000 | ステー GH-3000 | 数量3（うち1個は出庫後キャンセルで在庫に戻っている） |
| 通常品 | JKL-450 | プレート JKL-450 | 数量20（入荷日未確定） |
| 通常品 | XYZ-500 | カバー XYZ-500 | 在庫・履歴とも未使用（商品削除機能の確認用） |
| フィルター品 | F-100 | エアフィルター F-100 | 製造番号 FS-0001（在庫）・FS-0004（出庫済み） |
| フィルター品 | F-200 | オイルフィルター F-200 | 製造番号 FS-0002（在庫中、入荷日未確定） |
| フィルター品 | F-300 | 燃料フィルター F-300 | 製造番号 FS-0003（在庫）・FS-0005（出庫後キャンセルで在庫に戻っている） |

出庫履歴には通常品「出庫済み1件」「キャンセル1件」、フィルター品「出庫済み1件」「キャンセル1件」が入っている。

## 本実装に進む場合の申し送り

- **QRコード読み取り**：`BarcodeDetector` API と `getUserMedia` で実装済み（フィルター在庫一覧・フィルター入庫の製造番号）。QRコードのみを対象とし、1Dバーコードは扱わない。`BarcodeDetector` が使えない環境（Windows/Mac の Chrome など）では `src/js/vendor/zxing.min.js`（[zxing-js](https://github.com/zxing-js/library)、Apache-2.0）にフォールバックする。誤読対策として同じ値が連続で読めたときだけ確定し、ネイティブ検出が機能しない端末では自動でZXingに切り替える。**スマートフォン実機・各種ブラウザでの読み取り精度は本実装時に改めて検証する**。
- **スマートフォンからのカメラ利用**：`getUserMedia` は `https`、または `http://localhost`（端末自身）でしか動かない。
  パソコンで起動したサーバーにスマートフォンから `http://（パソコンのIPアドレス）` でアクセスする構成では、
  ブラウザの仕様としてカメラを一切使わせないため、コード側での回避はできない。モックでは
  ...
  自己署名証明書のため初回アクセス時にブラウザの警告が出るが、進めば以降は使える。本実装では
  社内サーバー・トンネリングサービス等、正式な証明書での配信を検討する。
