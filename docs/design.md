...
出庫履歴・フィルター出庫履歴の「CSVダウンロード」は、列構成・順序を完全に一致させている
（商品コード・製品名・保管場所・数量・入荷日・出庫した人・出荷先コード・出荷先小番・
出荷先名1・出荷先名2・受注番号1〜3・備考・出庫日時・状態）。フィルター品には保管場所・数量の
概念が無いため、フィルター出庫履歴側は保管場所を常に空欄、数量を常に`1`にして列を埋めている
（フィルター品は1行＝1個のため）。製造番号はこの列構成には含まれない（列を完全に揃えることを
優先し、対象外にした）。
...
- 商品が複数ある出庫操作は、まとめ表示の見出し行（「▶ 3件の商品をまとめて出庫」）にする。
  クリックすると開閉し（`expandedGroups` に開閉状態を保持）、展開すると商品ごとの内訳行が
  インデントして表示される。内訳行にはそれぞれの商品コード・数量などとキャンセル/削除ボタンが
  あり、まとめ表示の中からでも個々の商品単位でキャンセル・削除できる（`cancelShipment`/
  `deleteShipment` は元々1件の出庫記録単位で動くため、まとめ表示はあくまで見た目の集約であり、
  データや操作の単位はこれまで通り1商品＝1レコードのまま）。
