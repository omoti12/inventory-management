...
```
src/index.html            15画面ぶんの section とタブナビ（月次締め・ヘルプはタブでなくヘッダーのボタンから開く）
...
`App.views[name]`には`onShow`だけでなく`onHide`も登録できる。`ui.js`の`showView(name)`は
切り替え前の画面が変わる時（同じ画面への呼び出しでは呼ばない）、切り替え後に前の画面の
`onHide`があれば呼ぶ。用途は「入力途中でも、その画面を離れたら残さず消したいフォーム」で、
今のところ4画面（`inbound.js`/`filter-inbound.js`/`shipping.js`/`filter-shipping.js`）が
`App.views[name].onHide = resetForm`という同じ形で使っている。入庫・出庫はどちらも他の画面
（在庫一覧・商品管理など）を頻繁に行き来しながら使うため、入力途中のまま他画面に移動して
戻ってくると古い入力が残っていて紛らわしい、という声を受けて追加した。出庫・フィルター出庫の
`resetForm()`はフォームの入力欄だけを消し、対象商品の選択（`targetIds`、在庫一覧から
「選択した商品を出庫へ」で渡された分）はそのまま保持する（選び直しさせられると手間なため）。

`onShow`のタイミングでリセットしなかったのには理由がある。`onShow`は以下の2箇所でも
使われており、そこでリセットしてしまうと副作用が出る。

- **在庫一覧の「コピー」機能**（`inventory.js`が`App.inbound.startCopy(item)`で入力欄を
  埋めた直後に`App.ui.showView('inbound')`を呼ぶ）。`onShow`でリセットすると、コピーした
  内容が表示した瞬間に消えてしまう。`onHide`は「離れる時」にしか発火しないため、この
  流れ（他画面→入庫画面）には影響しない。
- **ヘッダーの「更新」ボタン**（`App.ui.refreshCurrentView()`は今表示中の画面の`onShow`を
  呼び直す）。入庫・出庫画面を開いたまま更新ボタンを押しても、入力中の内容が消えては困る。
  `refreshCurrentView()`は`onHide`を呼ばない（画面を離れるわけではないため）ので、
  この場合も入力は保持される。
...
| ヘルプ | システムの使い方の説明（基本の流れ・各画面の説明・よく使う操作・困った時の対処）。静的なテキストのみ、専用のJSモジュール無し |
...
- **出庫・フィルター出庫フォーム**: 出荷先コード欄・出荷先名1欄それぞれに`<datalist>`
  ...
  - 出荷先名1を**空にした**場合（3.の入り口）は、`syncDestinationFromName()`が出荷先コード・
    小番・出荷先名2も一緒に空にする。誤った出荷先を選んでしまい出荷先名1を消して選び直そうと
    した時、古いコード・小番・名2が残ったままだと「名前は空欄なのにコードは古いまま」という
    食い違った状態になり、そのまま出庫してしまう事故につながるため。出荷先コード側（1.の入り口）
...
`itemToFields`が`stockType !== 'filter'`の時だけこれらを読み書きしていた）。「フィルター
入庫にも入庫した人を追加してほしい」という要望を受け、`receivedBy`だけをこの条件分岐から
外し、通常品・フィルター品共通で読み書きするように変更した（`quantity`/`orderNo`は
...
### 選択して一括削除：入庫履歴・フィルター入庫履歴・商品管理・フィルター商品管理・出荷先マスタ

出庫履歴のまとめ表示（グループ単位の一括キャンセル/削除、`history.js`の`onBulkCancel`/
`onBulkDelete`）とは別に、行ごとにチェックボックスを付けて自由に複数選択し、まとめて
削除できるようにした画面が5つある。各画面のJSモジュール（`inbound-history.js` /
`filter-inbound-history.js` / `products.js` / `filter-products.js` / `destinations.js`）
はどれも同じ形の実装をしている（多少の重複はあるが、`summarizeCodes()`など既存の
軽量なヘルパーの重複と同じ方針で許容している）。

- 表のヘッダーに「すべて選択」チェックボックス（`.col-check`列、先頭列）、`.head-actions`
  内に「選択した◯件を削除」ボタン（`.btn--danger`、選択0件の間は`disabled`）を置く。
- 選択状態はモジュール内の`selectedIds`（idをキーにした真偽値マップ）で持ち、検索・
  絞り込みで再描画されても選択が消えないようにする（出庫履歴の`selectedGroups`と同じ方針）。
  行を削除した後や、モーダルでキャンセルされた場合以外は`render()`のたびに
  `syncSelectionUi()`でヘッダーのチェック状態（未選択/一部選択=indeterminate/全選択）と
  ボタンの有効・無効・件数表示を選択状況に合わせ直す。
- 削除は`runBulk(ids, action)`（`history.js`の同名ヘルパーと同じ実装。1件ずつ順番に
  Store操作を呼び、成功件数と最初のエラーメッセージだけを拾う）で行う。一部だけ失敗しても
  （後述）成功した分は反映しつつ、最初のエラーだけトーストで伝える。
- **入庫履歴・フィルター入庫履歴**: 個別の「削除」ボタンと全く同じ条件
  （`row.status === 'in_stock' || row.productDeleted`）の行だけチェックボックスを出す。
  条件を満たさない行（出庫済みで商品が存在する行）はチェックボックス自体を出さない
  （選択できてしまうと一括削除時にその分だけ失敗として返ってきて紛らわしいため、
  個別削除ボタンの可否と一致させている）。「すべて選択」もこの条件を満たす行だけを対象にする。
- **商品管理・フィルター商品管理**: 削除に状態的な制約が無いため、全行にチェックボックスを
  出す。ただし使用中の商品が含まれる場合は、個別削除の確認ダイアログと同様に、選択した
  商品全体の使用状況（使用中の件数、在庫・出庫済みの合計数）を集計してから確認メッセージに
  含める（1件ずつ別々の確認は出さず、まとめて1つの確認ダイアログで済ませる）。
- **出荷先マスタ**: 出荷先には使用状況の概念が無いため、単純に選択件数だけを確認して削除する。
...
一方で、製造番号の末尾が数字の連番になっている場合（例：`62---100007`）は、同じ番号を
複製するのではなく1件ごとに番号を増やして登録したいことがある。そのため数量欄の下に
「製造番号の末尾を連番にして登録する」チェックボックスを追加した（既定はオフ＝従来通りの
複製登録）。チェックを入れて数量に`15`を指定すると、`62---100007`〜`62---100021`の15件が
登録される。`store.js`の`nextSerialNo(base, offset)`が製造番号末尾の数字部分を
`offset`だけ増やす（0埋めの桁数は維持しつつ、繰り上がりで桁が増える場合はそのまま桁を増やす）。
連番チェックを入れたのに製造番号の末尾が数字でない場合は、登録前にエラーで弾く
（`errors.sequential`）。複製登録と連番登録はどちらも同じ`addFilterItem()`の中で
`sequential`フラグの有無だけで分岐しており、既存の複製登録の挙動（連番チェックを入れない
場合）には影響しない。

### フィルター入庫：製造番号の重複警告

末尾が連番の製造番号（例：`62---100007`）は、前回どこまで登録したか忘れて同じ番号から
また登録してしまう間違いが起きやすい、というフィードバックを受けて、登録前に重複
チェックを追加した。`store.js`の`findFilterItemsBySerialNo(serialNo)`が、在庫中・出庫済み
を問わず全フィルター品から完全一致する製造番号を探して返す。`filter-inbound.js`の
`onSubmit()`は、フォームの送信時にまずこのチェックを行い：

- 一致が無ければそのまま`submitRegistration(data)`（実際の登録処理。従来の
  `onSubmit()`の中身をそのまま切り出したもの）を呼ぶ。
- 一致があれば、既存の1件目の商品コード・製品名・状態（在庫中/出庫済み）を含めた
  確認ダイアログ（「製造番号「62---100007」は既に登録されています（...）。それでも
  登録しますか？」）を出し、承認された場合だけ`submitRegistration(data)`を呼ぶ。
  キャンセルすれば何も登録しない。

同じ製造番号を意図的にまとめて登録する運用（数量欄での複製登録）は引き続きでき、
このチェックは登録をブロックするものではなく、確認を挟むだけ。連番登録
（`sequential`が真）の場合は、ユーザーが入力した先頭の製造番号だけをチェックする
（自動生成される2件目以降まではチェックしない。誤りが起きやすいのは手入力した
先頭の番号のため）。

送信時の確認ダイアログだけでは「登録するボタンを押すまで重複に気づけない」という
フィードバックを受け、製造番号欄に**入力した時点**（`input`イベント。スキャンでの
自動入力も対象。`scan-btn`のクリックハンドラが読み取り後に`input`イベントを手動発火
させているため、同じ仕組みに乗る）でも同じチェックを行い、欄のすぐ下に注意書き
（`#filter-inbound-serial-duplicate-hint`、`.form-note--warn`）を出すようにした
（`filter-inbound.js`の`updateDuplicateHint()`）。こちらは登録をブロックしないだけで
なく確認も挟まない、純粋な早期の気付き用。実際にその番号のまま登録しようとした時の
最終防波堤は、引き続き送信時の確認ダイアログが担う。フォームをリセットする時
（`resetForm()`）は、この注意書きも一緒に消す。
...
上の「在庫を検索して追加」は絞り込み結果を見てから数量を入力して「追加」を押す2段階の
操作で、バーコードを読み取ってその場で1件だけ出庫対象に加えたい場合には手数が多い。
そのため「出庫情報」カードの一番上に、製造番号を完全一致で探してその1件だけを即座に
出庫対象へ追加する「製造番号ですぐ追加」欄を別途用意した（`filter-shipping.js`の
`quickAddBySerialNo()`）。

- `store.js`の`findInStockFilterItemsBySerialNo(serialNo)`で完全一致・在庫中のものだけを探す
  （「在庫を検索して追加」側の`matchesRow`は部分一致だが、こちらは1件を確実に特定したいため
  完全一致にしている）。見つからなければエラートースト、既に出庫対象に追加済みならその旨の
  トーストを出す。
- この入力欄は`#filter-shipping-form`の中にあるが`name`属性を持たせていない。`values()`は
  `FormData`から名前で個々の値を取り出す実装のため、無名の入力欄があっても出庫データには
  混ざらない。
- バーコードスキャナーは読み取った値の後にEnterキーまで送ってくる機種が多い。この欄で
  Enterを押すと通常は`<form>`の送信（＝出庫する）が発生してしまうため、`keydown`で
  `Enter`だけを横取りして`quickAddBySerialNo()`を呼び、`preventDefault()`で送信を止めている。
  スキャンボタン・手入力どちらの経路でも同じ関数を呼ぶ。
...
- **タブには置かず、ヘッダーのボタンから開く**：当初は「その他」というタブグループを新設して
  ...
  アクティブ状態を示さなくても実用上問題ない。後から追加した「ヘルプ」画面（後述）も
  同じ理由・同じ方式でヘッダーに置いている。

## ヘルプ画面

使い方の説明だけの静的なページ。月次締めと同じ理由で、タブではなくヘッダーの
「❓ ヘルプ」ボタン（`#app-help`）から開く。`App.views.help`のような専用登録は無く、
`ui.js`の`VIEWS`配列に`'help'`を加え、`index.html`に`#view-help`のセクションを用意する
だけで、既存の`showView()`の仕組みにそのまま乗る（`App.views['help']`が無くても
`showView()`は`onShow`の呼び出しを安全にスキップする）。説明文だけのカードを並べる
構造のため、フォームや検索欄用の`.card__head`/`.form`とは別に、`.card__body`
（内側にパディングを持つ、見出し・段落・リスト用の汎用クラス）をCSSに用意している。

長くなってきて読みにくい、というフィードバックを受けて以下を追加した:
- **目次（`.help-toc`）**：見出し上部に、各カードへのアンカーリンク（`#help-xxx`）を
  ピル型ボタンで並べた簡易目次を置いた。クリックするとネイティブのアンカージャンプで
  該当カードまで飛ぶ（JS実装は無し）。ヘッダーが`position: sticky`で常に上部に
  表示され続けるため、ジャンプ先の見出しがヘッダーの下に隠れないよう、
  `.card[id] { scroll-margin-top: 180px; }`でアンカー位置に余白を持たせている。
- **カード見出しを`.card__head`/`.card__title`に統一**：他の画面のカードと同じ
  見出しバー（下線区切り）にして、視覚的な区切りを他画面と揃えた（以前は
  `.card__body`の中に地の文として`<h3>`を置いていた）。
- **「各画面の説明」を箇条書きから表に変更**：画面名と説明を2列の表（`.table`、他画面の
  一覧テーブルと同じスタイル）にして、画面名が探しやすいようにした。
- **「基本の流れ」の番号を丸バッジ表示に**：`.help-steps`で`<ol>`の既定のマーカーを消し、
  CSSカウンター（`counter-reset`/`counter-increment`）で連番の丸バッジを疑似要素
  （`::before`）で描画している。
- **「困ったときは」を目立つ枠で囲む**：`.help-callout`（アクセントカラーの背景・枠線）で
  トラブルシューティング項目全体を囲み、他の説明文よりも視覚的に目立たせた。
- 月次締めの説明に、判定基準（登録日時であって入荷日・出庫日ではないこと）についての
  補足を追記した（実際にユーザーがこの点で混乱した経緯があったため）。

## パソコン・タブレットでの見やすさ・使いやすさ

タブレット（特にiPad）で「プルダウンが見づらい」というフィードバックを受けて、
フォーム部品（`input[type=text/month/date/number]`・`textarea`・`select`）の見た目を
まとめて調整した。

- **フォントサイズを16px以上にする**：ページ全体の基準フォントサイズは14pxだが、
  フォーム部品は`font-size: 16px`を明示的に指定している。iOS Safariは、フォーカスした
  input/selectのフォントサイズが16px未満だと自動的にズームしてしまう仕様があり、これが
  「タブレットで見づらい」の主な原因だった可能性が高い。16px以上にすることでこの自動
  ズームを防ぐ。
- **タップしやすい高さを確保する**：`min-height: 44px`・`padding: 10px`を指定し、
  指でタップしやすい高さ（44px前後、Appleのヒューマンインターフェースガイドラインの
  目安）を確保した。
- 影響範囲はテキスト入力・日付/月入力・数値入力・テキストエリア・セレクトボックス全般
  （出庫した人・入庫した人の固定プルダウンや、商品コード等の`<datalist>`付き入力欄も含む）。
  ボタン（`.btn`/`.btn--sm`）やチェックボックスは対象外（`.btn--sm`はテーブルの操作列など
  余白が限られる場所で多用しており、広げるとテーブルレイアウトが崩れるリスクがあるため
  現状維持）。

## 登録ボタンの二重送信防止

Graphへの書き込みは非同期のため、「登録しました」の緑のトースト表示が出るまでの一瞬の間に
もう一度ボタンを押せてしまい、全く同じ内容が二重に登録されてしまう不具合があった
（`ship()`は在庫更新のETag競合こそ検知するが、全く別の新規Shipmentレコードとしての
二重作成そのものは防げない。`addItem()`/`addFilterItem()`/`addProduct()`等も同様に、
呼ばれた回数だけ新しいレコードを作る）。月次締め（`month-locks.js`）は元々この対策が
入っていたが、他の登録系フォームには無かったため、同じパターンで揃えた。

**やり方**：Store呼び出しの直前で送信ボタンを`disabled = true`にし、Promiseの解決
（成功・失敗どちらでも）の先頭で`disabled = false`に戻す。
...
これだけで十分な理由：
- ボタンの`disabled`属性は同期的に効くため、連打（同じ`click`イベントの中で2回目を
  クリックしようとしても、ブラウザはdisabledな要素にclickイベントを配送しない）を
  確実に防げる。
- `<form>`の暗黙的送信（Enterキー）も、有効な送信ボタンが無ければ発生しないため、
  同じ対策でEnterキー連打による二重送信も防げる。
- 出庫・フィルター出庫（`shipping.js`/`filter-shipping.js`）は送信前に確認ダイアログ
  （`App.ui.confirm()`、`<dialog>`の`showModal()`によるネイティブモーダル）を挟むため、
  「確認ダイアログが開いている間」の二重クリックはモーダル自体が背面の操作を無効化して
  くれる。実際に危険なのは「確認ダイアログでOKを押した直後〜`ship()`が解決するまで」の間
  なので、`disabled = true`はそのタイミング（`approved`が真になった直後、`ship()`を呼ぶ
  直前）に置く。

対象にしたのは登録・更新系の7画面：入庫・フィルター入庫・出庫・フィルター出庫・
商品管理・フィルター商品管理・出荷先マスタ（月次締めは元から対策済み）。入庫履歴等の
一覧インライン編集の保存ボタンや、削除・キャンセル系のボタンは対象にしていない
（同じ問題が理論上はあり得るが、今回のフィードバックは「登録」操作についてだったため、
まずは登録系フォームに絞った）。

検証は実際のクリック連打ではなく、モックのGraph呼び出しにわざと遅延を入れて再現した
（即座に解決するPromiseだと、2回目のクリックが発生する前に1回目が完了してしまい、
問題を再現できないため）。

## 今後の申し送り

- **バーコード読み取り**：`BarcodeDetector` API と `getUserMedia` で実装済み（フィルター在庫一覧・フィルター入庫・フィルター出庫の製造番号欄）。1Dバーコード各種とQRコードの両方を対象とする。`BarcodeDetector` が使えない環境（Windows/Mac の Chrome など）では `src/js/vendor/zxing.min.js`（[zxing-js](https://github.com/zxing-js/library)、Apache-2.0）にフォールバックする。誤読対策として同じ値が連続で読めたときだけ確定し、ネイティブ検出が機能しない端末では自動でZXingに切り替える。
