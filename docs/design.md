...
選択状態は画面内のモジュール変数で保持し、出庫完了後にクリアする。

`shipping.js`（通常の出庫）と同じ「在庫を検索して追加」欄（商品コード・製品名・製造番号で
絞り込み、`renderSearch()`が対象未追加のフィルター在庫を一覧表示）も追加した。フィルター品は
数量の概念が無く1行＝1個のため、`shipping.js`のように出荷したい数を入力する仕組みは無く、
検索結果の各行の「追加」ボタンでその1件（1製造番号）をそのまま出庫対象に加える。
