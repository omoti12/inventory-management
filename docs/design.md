...
- バーコード読み取り（カメラ）を使う場合のみ、`https`、または `http://localhost` などの**セキュアなコンテキスト**が必要。
  ...
  `scanner.js` の `describeError()` がこの場合に理由を案内する。
...
| 画面 | 主な操作 |
| --- | --- |
| 在庫一覧 | 検索（商品コードはバーコード読み取りにも対応）、明細/商品まとめの切替、行の選択、コピー、選択した商品を出庫へ（起動時に最初に表示） |
...
`inventory.js` の `inventory-scan-btn` クリックで `App.scanner.open()` を呼び、読み取った値を
商品コード検索欄にセットして再描画する（`inbound.js` / `filter-inbound.js` と同じ `App.scanner` を使う）。
...
- **バーコード読み取り**：`BarcodeDetector` API と `getUserMedia` で実装済み（在庫一覧の商品コード検索・フィルター入庫の製造番号）。`BarcodeDetector` が使えない環境（Windows/Mac の Chrome など）では `src/js/vendor/zxing.min.js`（[zxing-js](https://github.com/zxing-js/library)、Apache-2.0）にフォールバックする。**スマートフォン実機・各種ブラウザでの読み取り精度は本実装時に改めて検証する**。
- **スマートフォンからのカメラ利用**：`getUserMedia` は `https`、または `http://localhost`（端末自身）でしか動かない。
  パソコンで起動したサーバーにスマートフォンから `http://（パソコンのIPアドレス）` でアクセスする構成では、
  ブラウザの仕様としてカメラを一切使わせないため、コード側での回避はできない。本実装では
  スマートフォンからも安全にアクセスできるよう HTTPS 配信（社内サーバー・トンネリングサービス等）を用意する。
