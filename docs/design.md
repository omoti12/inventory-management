...
- 必須項目（通常品は`出庫した人`のみ＝`App.store.NORMAL_SHIPMENT_FIELDS`。フィルター品は
  従来通り出庫した人・受注先・エンドユーザーの3つ＝`App.store.SHIPMENT_FIELDS`）のいずれかが空、
  または対象商品が0個 → ボタンを `disabled`
- 未入力があるときは「未入力：出庫した人、受注先」のように項目名を表示
- 入力欄から離れた時点で、その項目が空ならエラーを表示

`store.js` の `ship(itemIds, info, requiredFields)` の第3引数で必須項目リストを差し替えられる
（省略時は`SHIPMENT_FIELDS`）。`shipping.js`は`NORMAL_SHIPMENT_FIELDS`を明示的に渡し、
`filter-shipping.js`は省略してデフォルトのまま使う。
...
入力欄を廃止した（出荷先コード・名で代替できるようになったため）。出庫履歴（通常品側）の一覧も
その2列を「出荷先名1」「出荷先名2」に差し替えている。データとしての`orderTo`/`endUser`自体は
`shipment`の形状に残しており、過去の記録は引き続き参照できる（新規の通常品出庫では単に空になる）。
フィルター出庫は変更していない（受注先・エンドユーザーとも引き続き必須）。汎用の「CSVダウンロード」
（`onExportCsv()`）は受注先・エンドユーザーの列をそのまま残しているため、通常品の新しい出庫では
その列が空欄になる。
